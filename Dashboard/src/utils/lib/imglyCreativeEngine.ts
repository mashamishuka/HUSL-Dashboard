...
export const zoomToSelectedText = async (engine: any, paddingTop = 0, paddingBottom = 0) => {
  ...
  const overlapBottom = Math.max(
    (canvasBounding.top + canvasBounding.height - (window.visualViewport as any).height) * window.devicePixelRatio,
    0
  )
  ...
  if (selectedTexts.length === 1) {
    ...
    const cursorPosYCanvas =
      pixelToCanvasUnit(engine, engine.editor.getTextCursorPositionInScreenSpaceY()) +
      engine.block.getPositionY(camera) -
      pixelToCanvasUnit(engine, visiblePageAreaY)
    if (cursorPosY > visiblePageAreaY || cursorPosY < paddingTop * window.devicePixelRatio) {
      engine.block.setPositionY(camera, cursorPosYCanvas)
    }
  }
}

export const pixelToCanvasUnit = (engine: any, pixel: number) => {
  const sceneUnit = engine.block.getEnum(engine.scene.get(), 'scene/designUnit')
  let densityFactor = 1
  if (sceneUnit === 'Millimeter') {
    densityFactor = engine.block.getFloat(engine.scene.get(), 'scene/dpi') / 25.4
  }
  if (sceneUnit === 'Inch') {
    densityFactor = engine.block.getFloat(engine.scene.get(), 'scene/dpi')
  }
  return pixel / (window.devicePixelRatio * densityFactor * engine.scene.getZoomLevel())
}
...
export const autoPlaceBlockOnPage = (
  ...
  engine.block.findAllSelected().forEach((blockId: number) => engine.block.setSelected(blockId, false))
...
export const replaceImage = (engine: any, block: any, imageFileURI: string, addUndoStep = true) => {
...
export const addImage = async (engine: any, parentId: number, imageURI: string, baseSize = 0.5) => {
  ...
  const [parentWidth, parentHeight] = [engine.block.getWidth(parentId), engine.block.getHeight(parentId)]
...
export function hexToRgb(hex: any) {
  return {
    r: ('0x' + hex[1] + hex[2]) as string | 0,
    g: ('0x' + hex[3] + hex[4]) as string | 0,
    b: ('0x' + hex[5] + hex[6]) as string | 0
  }
}
export function rgbToHex(r: number, g: number, b: number) {
  return '#' + ((1 << 24) + (r << 16) + (g << 8) + b).toString(16).slice(1)
}
// export hex to rgb but with values between 0 and 1
export const hextToRgbRanged = (hex: any) => {
  let c: any
...
export const normalizeColors = ({ r, g, b }: any) => ({
  r: r / 255,
  g: g / 255,
  b: b / 255
})
export const denormalizeColors = ({ r, g, b }: any) => ({
  r: r * 255,
  g: g * 255,
  b: b * 255
})
 ...
 */
export const isColorEqual = (colorA: any, colorB: any, precision = PRECISION) => {
  return (
...
export const RGBAArrayToObj = ([r, g, b]: any) => ({ r, g, b })

const isFullSizeBlock = (engine: any, pageBlockId: number, blockId: number) => {
  const tolerance = Math.max(engine.block.getWidth(pageBlockId), engine.block.getHeight(pageBlockId)) * 0.1
  const isInTolerance = (numberToCheck: number, expectedNumber: number) =>
  ...
  return (
    ...
    isInTolerance(engine.block.getPositionX(blockId) + engine.block.getWidth(blockId), engine.block.getWidth(pageBlockId)) &&
    isInTolerance(engine.block.getPositionY(blockId) + engine.block.getHeight(blockId), engine.block.getHeight(pageBlockId))
  )
}

const getFramePositionX = (engine: any, blockId: number) => {
...
const getFramePositionY = (engine: any, blockId: number) => {
 ...
 * @param {Number} height
 */
export const resizeCanvas = (engine: any, pageBlockId: number, width = 1080, height = 1080) => {
  const blocksOnPage = engine.block.getChildren(pageBlockId)
  const fullSizeBlocks: any[] = []
  const otherBlocks: any[] = []
  blocksOnPage.forEach((blockId: number) => {
  ...
  const groupX2 = engine.block.getGlobalBoundingBoxWidth(transformGroupId) + groupX1
  const groupY2 = engine.block.getGlobalBoundingBoxHeight(transformGroupId) + groupY1
  ...
  const calculateDistance = (x1: number, x2: number, y1: number, y2: number) => Math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2)
  const groupBBpointsWithCenterDistance = groupBBpoints.map(([X, Y]) => ({
    distance: calculateDistance(pageCenterX, X, pageCenterY, Y),
    point: [X, Y]
  }))
  const groupBBpointsWithCenterDistanceSorted = groupBBpointsWithCenterDistance.sort((a, b) => -a.distance + b.distance)
  const pointWithBiggestDistance = groupBBpointsWithCenterDistanceSorted[0].point
  const mirroredPoint = [
  ...
  const scaleFactor = newAspectRatio > currentRatio ? height / groupHeight : width / groupWidth
  ...
  engine.block.setPositionY(transformGroupId, height / 2 - finalGroupHeight / 2)
