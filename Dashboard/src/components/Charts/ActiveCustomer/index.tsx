...
export const ActiveCustomerChart = () => {
  const { data, error } = useSWR<RestApi.Response<any>>('/businesses/overview')

  const customerReport = useMemo(() => {
    if (!data) return null

    const dataset = data?.data?.activeCustomers?.data
    const labels = data?.data?.activeCustomers?.labels?.map((v: any) => moment().subtract(v, 'days').format('MMM D'))
    return {
      labels,
      dataset
    }
