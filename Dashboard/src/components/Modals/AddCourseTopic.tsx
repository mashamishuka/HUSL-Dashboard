...
import { createTopic } from '@src/restapi/course/mutations'
...
export const AddCourseTopic: React.FC<DefaultModal> = ({ show, onClose, data, courseId, mutate, width = '42rem' }) => {
  const [uploadType, setUploadType] = useState<'youtube' | 'manual'>('manual')

  const handleAddTopic = async (values: Record<string, any>) => {
    ...
    try {
      ...
      await createTopic(courseId, data?._id, values)
                  ...
                  {({ handleSubmit, handleBlur, handleChange, values, isSubmitting, setFieldValue }) => (
                    <form onSubmit={handleSubmit} className="flex flex-col mt-5 space-y-5">
                      ...
                      <Dropzone
                        ...
                        required
                      />
                      <TextEditor label="Content" name="content" setFieldValue={setFieldValue} height={250} />
