...
import { createOnboardingItemSchema } from './schema'
...
import { createOnboardingItem } from '@src/restapi/onboardings/mutation'
...
export const CreateOnboardingItemForm: React.FC = () => {
  ...
  const handleSaveOnboardingItem = async (values: Record<string, any>) => {
    try {
      // remove empty object value from values
      ...
      const data = await createOnboardingItem(body)
      if (data?.status === 201) {
  ...
  return (
    <Formik
      ...
      onSubmit={handleSaveOnboardingItem}
      validationSchema={createOnboardingItemSchema}
      enableReinitialize>
            ...
            <div className="flex flex-col my-3 space-y-2">
              {actions?.map((action, index) => (
                <div key={index} className="flex w-full space-x-3">
                  ...
                  <button onClick={() => handledeleteAction(index)} className="flex justify-center mt-3 text-lg text-danger">
                    <MdDelete />
                  </button>
                </div>
              ))}
